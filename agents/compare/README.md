...
```
make agent-compare                                   # from the repo root
cd agents/compare && uv run python -m vdagent_compare     # same, by hand
```
...
Configure it in `agents/compare/.env` (gitignored; wins over the process env, then the repo-root

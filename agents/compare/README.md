...
```
make agent-compare                                   # dials the Backend's hub (VDAGENT_BACKEND)
uv run python -m vdagent_compare                     # same, by hand
```
...
Environment: `agents/compare/.env` (wins over everything), then the process env, then the repo-root
`.env`.

| Variable | |
|---|---|
| `VDAGENT_AGENT_TOKEN_COMPARE` | Required. Hub token; the Backend must have the same value. Generate with `python -c "import secrets; print(secrets.token_urlsafe(24))"`. |
